...
const CV = () => {
  ...
  const currentPositions = cv.currentPositions as Array<{
    title: string; institution: string; period: string; location: string; type: string;
  }>;
  const keyAffiliations: string[] = cv.keyAffiliations;
  const recentAwards = cv.recentAwards;
  const majorGrants = cv.majorGrants;
  const professionalOrgs: string[] = cv.professionalOrgs;
  ...
  const _education_removed = [
    ...
    {
      degree: 'Bachelor of Science in Environmental Science (Natural Resource Management)',
  ...
  const _currentPositions_removed = [
    {
    ...
    },
    {
      ...
      period: '',
      location: (
        <ul className="list-disc pl-5 space-y-1 text-sm text-stone-500">
          <li>Center for Biodiversity Outcomes</li>
          <li>Global Institute of Sustainability and Innovation / School of Life Sciences</li>
  ...
  const _keyAffiliations_removed = [
  ...
  const _publicationHighlights_removed = [
    {
  ...
  const _professionalOrgs_removed = [
    ...
    <div className="pt-16">
      ...
      <section className="py-20 bg-gradient-to-br from-stone-900 to-amber-900 text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex justify-center space-x-4"> 
            <a
              href={(cv as any).googleDocViewUrl || 'https://docs.google.com/document/d/1MbFcdQGCV3MngNR5t6NCFMYhYOQtHUNG/preview'}
              target="_blank"
            ...
            <a
              href={(cv as any).googleDocPdfUrl || 'https://docs.google.com/document/d/1MbFcdQGCV3MngNR5t6NCFMYhYOQtHUNG/export?format=pdf'}
              target="_blank"
              rel="noopener noreferrer"
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="text-lg text-stone-700 leading-relaxed">
          <p className="mb-2">{cv.researchInterests || 'Police\u2013community relations, social psychological processes and legal compliance (e.g., legitimacy, obligation to obey), environmental crime, conservation criminology, criminological theory, research methodology.'}</p>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 space-y-4">
                ...
                <div key={index} className="border-l-4 border-amber-600 pl-4">
                  ...
                  {item.details && item.details.trim() !== "" && (
                    <p className="mt-2 text-stone-700 italic">
                      <a 
                        href="https://academicworks.cuny.edu/cgi/viewcontent.cgi?article=5072&context=gc_etds" 
                        target="_blank" 
                        rel="noopener noreferrer"
                  ...
                  {item.honors && (
                    <p className="mt-1 text-amber-700 font-medium">
                      {item.honors.replace(/^Honors:\s*/i, '')}
                    </p>
                  )}
                </div>
              ))}
            ...
            <div className="p-6 bg-stone-50 border border-t-0 rounded-b-xl space-y-6">
              ...
              {currentPositions.map((item, index) => (
                <div key={index} className={`border-l-4 pl-4 ${item.type === 'primary' ? 'border-amber-600' : 'border-stone-400'}`}>
                  ...
                  <div className="flex items-center space-x-4 text-sm text-stone-500 mt-1">
                    {item.period && <span className="flex items-center space-x-1"><Calendar className="h-4 w-4" /> <span>{item.period}</span></span>}
                    {item.location && <span className="flex items-center space-x-1"><MapPin className="h-4 w-4" /> <span>{item.location}</span></span>}
                  </div>
                </div>
              ))}
              ...
              <ul className="list-disc pl-5 space-y-1 text-stone-700">
                {keyAffiliations.map((aff, index) => {
                  ...
                  if (aff.startsWith("Program Lead for Justice and Equity in Conservation")) {
                    href = "https://sustainability-innovation.asu.edu/biodiversityoutcomes/people/";
                    label = "Program Lead for Justice and Equity in Conservation";
                  } else if (aff.startsWith("Research and Evaluation Center")) {
                    href = "https://johnjayrec.nyc/people/";
                    label = "Research and Evaluation Center";
                  } else if (aff.startsWith("Christian Regenhard Center for Emergency Response Studies")) {
                    href = "https://christianregenhardcenter.org/staff";
                    label = "Christian Regenhard Center for Emergency Response Studies";
                  } else if (aff.startsWith("Center on Public Security")) {
                    href = "https://www.rutgerscps.org/people.html";
                    label = "Center on Public Security";
                  } else if (aff.startsWith("Academy for Justice")) {
                    href = "https://academyforjustice.asu.edu/";
                    label = "Academy for Justice";
                  } else if (aff.startsWith("Center for Violence Prevention and Community Safety")) {
                    href = "https://cvpcs.asu.edu/about/faculty-and-staff";
                    label = "Center for Violence Prevention and Community Safety";
                  }
        ...
        <div>
          ...
          {expandedSections.grants && (
            <div className="p-6 bg-stone-50 border border-t-0 rounded-b-xl space-y-6">
              <div className="relative overflow-hidden rounded-xl border border-green-700 bg-gradient-to-r from-green-900 via-emerald-800 to-green-700 px-6 py-4 shadow-lg">
                <div className="absolute inset-0 opacity-10 bg-[radial-gradient(circle_at_top,_#ffffff,_transparent_60%)]" />
                <div className="relative flex items-baseline justify-between">
                  <div>
                    <p className="text-sm uppercase tracking-[0.18em] text-green-100">Total Grant Funding Secured</p>
                    <p className="mt-1 text-3xl font-extrabold text-white">$1,519,111</p>
                  </div>
                  <p className="max-w-xs text-right text-xs text-green-100/80">
                    As Principal Investigator, Co-Principal Investigator, or Lead Researcher for practitioner recipient.
                  </p>
                </div>
              </div>
              ...
              {majorGrants.map((item, index) => (
                <div key={index} className="border-l-4 border-green-600 pl-4">
                  ...
                  <div className="flex flex-wrap gap-x-4 text-sm text-stone-500 mt-1">
                    <span>
                      <span className="font-semibold text-stone-700">Amount:</span>{' '}
                      <span className="text-green-700 font-semibold">{item.amount}</span>
                    </span>
                    <span>
                      <span className="font-semibold text-stone-700">Role:</span> {item.role}
                    </span>
                    <span>
                      <span className="font-semibold text-stone-700">Period:</span> {item.period}
                    </span>
                  </div>
                </div>
              ))}
